...
async def test_tweet_post(client):
    response = await client.post("/api/tweets", headers={"api-key": "user1"}, json={"tweet_data": "Hello world111!", "tweet_media_ids": []})
...
async def test_wrong_type_post(client):
    response = await client.post('/api/tweets', headers={"api-key": "user1"}, data={"tweet_data": "Hello world111!", "tweet_media_ids": []})
    assert response.status_code == 422

@pytest.mark.asyncio
async def test_no_existing_media(client):
    response = await client.post("/api/tweets", headers={"api-key": "user1"}, json={"tweet_data": "Hello world111!", "tweet_media_ids": [1,2]})
...
async def test_create_and_delete_tweet(client):
    create_response = await client.post("/api/tweets", headers={"api-key": "user1"}, json={"tweet_data": "Hello world111!", "tweet_media_ids": []})
    ...
    delete_response = await client.delete(f"/api/tweets/3", headers={"api-key": "user1"})
...
async def test_liked_and_unliked_tweet(client):
    liked_response = await client.post('/api/tweets/2/likes', headers={"api-key": "user1"})
    ...
    unliked_response = await client.delete('/api/tweets/2/likes', headers={"api-key": "user1"})
    assert unliked_response.status_code == 200
    data = unliked_response.json()
    assert data['result'] == True

    unliked_check_response = await client.get('/api/tweets', headers={"api-key": "user2"})
...
async def test_following_and_unfollowed_user(client):
    following_response = await client.post('/api/users/1/follow', headers={"api-key": "user2"})
    assert following_response.status_code == 201
    data = following_response.json()
    assert data['result'] == True

    following_response_check = await client.get('/api/users/me', headers={"api-key": "user1"})
    ...
    unfollowing_response = await client.delete('/api/users/1/follow', headers={"api-key": "user2"})
    assert unfollowing_response.status_code == 200
    data = unfollowing_response.json()
    assert data['result'] == True

    unfollowing_response_check = await client.get('/api/users/me', headers={"api-key": "user1"})
...
async def test_add_media(client):
    ...
    add_tweet_response = await client.post('/api/tweets', headers={"api-key": "user1"}, json=({"tweet_data": "hello", "tweet_media_ids": [1]}))
...
async def test_chek_unautorized_user(client):
    ...
    tweet_post_response = await client.post('/api/tweets', headers={"api-key": 'dd'},
